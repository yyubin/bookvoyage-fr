...
export default function RecommendationList({
  ...
  const { refreshSession } = useRecommendationSession();

  const handleRefresh = async () => {
    const nextSessionId = refreshSession();
    setIsRefreshing(true);
    try {
      const page = await getBookRecommendations(
        { limit: Math.max(initialItems.length, 20), enableSampling: true },
    ...
    <>
      <div className="mt-6 flex flex-wrap items-center justify-end gap-3">
        <button
          ...
          className="rounded-full border border-[var(--border)] bg-white px-4 py-2 text-xs font-semibold text-[var(--muted)] transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60"
        >
          {isRefreshing ? "불러오는 중..." : "새로고침"}
        </button>
      </div>
      {items.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-dashed border-[var(--border)] bg-white/70 px-4 py-4 text-sm text-[var(--muted)]">
